...
| 順 | 内容 | 参照 |
|----|------|------|
| 1 | **匿名寄付→アカウント移行のモック**: getMe に `pending_token_migration: true` を返すケースを用意し、フロントで「これまでの寄付をアカウントに引き継ぎますか？」ダイアログを表示。`POST /api/me/migrate-from-token` モックを追加し、送信後にフラグを false に。 | 12.4 |
| 2 | **利用停止・凍結時のメッセージ**: 利用停止アカウント・凍結/削除プロジェクトに対してアクション試行時に、親切なメッセージを表示する UI（API の 403/404 を拾って文言表示）。 | idea.md・user-management-mock-plan 1.3 |
| 3 | **About の整備**: 公式 URL・GitHub リンク、悪意あるクローン運営について責任を負わない旨の記載（Phase 6 で実装検討）。 | idea.md・12.6 |
