...
- **寄付履歴のメッセージフィルタ**: プロジェクトオーナーが寄付履歴タブで `has_message` フィルタを使い、メッセージ付き寄付のみに絞り込める。旧「メッセージ」タブと `GET /api/projects/:id/messages` API は寄付履歴に統合し廃止。
...
- [ ] プロジェクトオーナーが寄付履歴タブで「メッセージあり」フィルタを使いメッセージ付き寄付のみ表示できる
- [ ] 寄付履歴の日時ソート・寄付者名フィルタ・ソースフィルタが機能する
