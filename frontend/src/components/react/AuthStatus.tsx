...
export default function AuthStatus({ locale = 'ja' }: Props) {
  ...
  const [migrationDismissedId, setMigrationDismissedId] = useState<string | null>(() =>
    typeof window !== 'undefined' ? window.sessionStorage?.getItem(MIGRATION_DISMISSED_KEY) : null
  );
  ...
  const handleMigrationConfirm = async () => {
    ...
    try {
      ...
      fetchUser();
      if (typeof window !== 'undefined') window.sessionStorage?.setItem(MIGRATION_DISMISSED_KEY, user.id);
  ...
  if (user) {
    ...
    const showMigrationDialog = user.pending_token_migration && user.id !== migrationDismissedId;
    return (
      <>
      {user.suspended && (
        <div
        ...
        >
          {locale === 'ja' ? 'このアカウントは利用停止されています。ご不明な点は運営までお問い合わせください。' : 'This account has been suspended. Please contact support if you have questions.'}
        </div>
      )}
      <div className="auth-status">
        {MOCK_MODE && (
      ...
      {showMigrationDialog && (
        <div
          ...
          onClick={(e) => e.target === e.currentTarget && handleMigrationLater()}
        >
          <div
            style={{
          ...
          >
            <h2 id="migration-dialog-title" style={{ margin: '0 0 1rem', fontSize: '1.1rem' }}>
              {locale === 'ja' ? 'これまでの寄付をアカウントに引き継ぎますか？' : 'Migrate your previous donations to this account?'}
            </h2>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
              <button type="button" className="btn" onClick={handleMigrationLater}>
                {locale === 'ja' ? '後で' : 'Later'}
              </button>
              <button type="button" className="btn btn-primary" onClick={handleMigrationConfirm} disabled={migrating}>
                {migrating ? (locale === 'ja' ? '処理中...' : 'Processing...') : (locale === 'ja' ? '引き継ぐ' : 'Migrate')}
              </button>
            </div>
          </div>
        </div>
