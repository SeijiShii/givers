...
import { getMe, getAdminUsers, getDisclosureExport, type User, type AdminUser } from '../../lib/api';
...
export default function AdminUserList({
  ...
  const [disclosureType, setDisclosureType] = useState<'user' | 'project'>('user');
  ...
  return (
    <div className="admin-users">
      ...
      <div className="card" style={{ marginTop: '1rem', padding: '1rem' }}>
        <h2 style={{ margin: '0 0 0.5rem', fontSize: '1rem' }}>{t(locale, 'admin.disclosureExportTitle')}</h2>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center' }}>
          <select
            ...
            onChange={(e) => setDisclosureType(e.target.value as 'user' | 'project')}
            style={{ padding: '0.35rem 0.5rem', borderRadius: '4px', border: '1px solid var(--color-border)' }}
          >
            <option value="user">{t(locale, 'admin.disclosureExportTypeUser')}</option>
            <option value="project">{t(locale, 'admin.disclosureExportTypeProject')}</option>
          </select>
          <input
            type="text"
            value={disclosureId}
          ...
          >
            {disclosureLoading ? (locale === 'ja' ? '出力中...' : 'Exporting...') : t(locale, 'admin.disclosureExportButton')}
          </button>
        </div>
        {disclosureError && (
          <p style={{ margin: '0.5rem 0 0', fontSize: '0.85rem', color: 'var(--color-danger)' }}>{disclosureError}</p>
        )}
      </div>
      {disclosureConfirmOpen && (
        ...
        >
          ...
          >
            <p style={{ margin: '0 0 1rem', fontSize: '0.9rem' }}>{t(locale, 'admin.disclosureExportNotice')}</p>
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
              <button type="button" className="btn" onClick={() => !disclosureLoading && setDisclosureConfirmOpen(false)} disabled={disclosureLoading}>
                {locale === 'ja' ? 'キャンセル' : 'Cancel'}
              </button>
              <button type="button" className="btn btn-accent" onClick={handleDisclosureExport} disabled={disclosureLoading}>
