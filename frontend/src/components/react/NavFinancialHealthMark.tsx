...
export default function NavFinancialHealthMark({ locale }: Props) {
  ...
  const titleNotReached = t(locale, 'nav.financialHealthTitleNotReached', { rate: String(rate) });
    ...
    >
      {reached ? (
        <>
          ...
          <span className="nav-financial-health-mark__text">{t(locale, 'nav.financialHealthReached')}</span>
        </>
      ) : (
        <>
          <span className="nav-financial-health-mark__icon" aria-hidden="true">
          ...
          <span className="nav-financial-health-mark__text">{t(locale, 'nav.financialHealthNotReached', { rate: String(rate) })}</span>
        </>
      )}
    </span>
  );
