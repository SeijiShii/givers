...
### 未整理
- DEV用サーバーを立てる
  - そちらに「開発用」という表示を付け、GIVErSへのリンクを表示する
  - Conohaサーバーインスタンスを立てる
  - サブドメ：　dev.givers.work
  - DEV環境用デプロイスクリプト
  - 本番環境のDBダンプを抜き出してローカルにDLするスクリプト

### バグレポート
- ~~ログインユーザーが定期的な寄付を申し込んで決済するとプロジェクト詳細に戻る。~~
  - ~~その時はサブスク寄付管理UIになっている必要がある。~~
  - **修正済み**: Checkout戻り後の1秒ディレイ内でサブスク状態を再取得するよう `ProjectDetail.tsx` を修正 (E2Eテスト追加済み)