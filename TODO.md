...
- 悪用の定義
  - マネロン対策
- ホストから特定のユーザーにメッセージを送信する仕組み。
  - アナウンスに宛先機能を追加する。ホストが宛先指定でユーザーにメッセージを送ったときのみチャットスレッドが開く。ホスト側の判断でチャットスレッドを活性不活性化できる。
- ホストがプロジェクト作成するとき「利用規約に同意する」は不要
- 非ログインユーザーが単発寄付をしたとき、同じブラウザからログインすると寄付履歴を統合するという仕様になっているはずだが、機能していない。
